...
const FeaturesCard: React.FC<FeatredCardProps> = ({ icon, title, content, index }) => (
  <div
    className={`flex flex-row w-[700px] h-[80px] p-2 rounded-[20px] ${index !== features.length - 1 ? "mb-1" : "mb-0"
      } feature-card`}
  >
    <div
      className={`w-[25px] h-[25px] rounded-full bg-dimBlue ${styles.flexCenter}`}
    >
      <Image src={icon} alt="icon" className="w-[40%] h-[40%] object-contain" />
    </div>
    <div className="flex-1 flex flex-col ml-3">
      <h4 className="font-poppins font-semibold text-white text-[14px] leading-[20px]">
        {title}
      </h4>
      <p className="font-poppins font-normal text-dimWhite text-[11px] leading-[24px]">
        {content}
      </p>
    </div>
  </div>
